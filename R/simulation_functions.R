...
#' Preprocesses the data for fitting a model
#'
#' This function preprocesses the data by filtering it based on the specified
#' start and end years, start and end months, and window start and end values.
#' It also computes smoothed versions of the population size and computes
#' covariate values based on the specified model.
#'
#' @param dat The input data frame containing the data to be preprocessed.
#' @param model The model type to be used for preprocessing.
...
#' @param start_month The start month for filtering the data.
#' @param end_month The end month for filtering the data.
#' @param window_start The start month of the window for computing covariate values.
#' @param window_end The end month of the window for computing covariate values.
#'
#' @return The preprocessed data frame.
#'
#' @details The function performs the following steps:
#'   - Computes the number of years in the data frame.
#'   - Creates a smoothed version of the population size.
#'   - Computes covariate values based on the specified model and window.
#'   - Filters the data frame based on the specified start and end years and months.
#'   - Normalizes the covariate values based on the specified model.
...
preprocess_data <- function(
  ...
  n_years <- max(dat$year) - (min(dat$year) - 1)

  if (!is.null(extra_cov)) {
    dat_extra_cov <- dat
  }

  ## creates smoothed version of population size
  spl_both <- loess(dat$pop ~ dat$time)$fitted
  spl_both <- smooth.spline(x = dat$time, spl_both)

  ## for models that use a yearly covariate value based on windows
  ## compute covariate values
  n_month_window <- window_end - window_start + 1
  if (model %in% c(
    "original", "original_s5",
    "const", "small",
    "original_normal", "sd",
    "exponent", "inf_exponent", "two_cov", "baseline"
  )) {
    dat <- dat |>
      group_by(year) |>
      mutate(is_window = if_else(month >= window_start &
        month <= window_end, 1, 0)) |>
      mutate(cov2 = sum(!!sym(covariate) * is_window) / n_month_window) |>
      ungroup()
  } else { ## if is not the case
    dat <- dat |> mutate(cov2 = dat[[covariate]])
  }
  # removed lag option
  dat <- dat |>
    mutate(covariate = cov2) |>
    drop_na(covariate)

  # filter dataset only for dplyr::selected years and months
  dat <- dat |>
    filter(year >= start_year, year <= end_year) |>
  ...
  dat <- mutate(dat,
    covariate = (cov2 - min(cov2)) / (max(cov2) - min(cov2)),
    pop = predict(spl_both, deriv = 0, x = dat$time)$y,
    dpopdt = predict(spl_both, deriv = 1, x = dat$time)$y
  ) |>
    dplyr::select(-cov2)

  if (!is.null(extra_cov)) {
    extra_cov_dat <- c()
    for (i in 1:nrow(extra_cov)) {
      cov_name <- extra_cov[i, "covariate"] |>
        unlist() |>
        unname()
      ws <- extra_cov[i, "window_start"] |>
        unlist() |>
        as.numeric() |>
        unname()
      we <- extra_cov[i, "window_end"] |>
        unlist() |>
        as.numeric() |>
        unname()
      glue_name <- paste0("cov", i)

      df_extra <- dat_extra_cov |>
        dplyr::select(c("year", "month", cov_name)) |>
        group_by(year) |>
        filter(month >= ws, month <= we) |>
        summarize("{glue_name}" := mean(!!sym(cov_name))) |>
        mutate("{glue_name}" := (!!sym(glue_name) - min(!!sym(glue_name))) / (max(!!sym(glue_name)) - min(!!sym(glue_name))))
      if (i == 1) {
        extra_cov_dat <- df_extra
      } else {
        extra_cov_dat <- extra_cov_dat |> left_join(df_extra)
      }
    }
    dat <- dat |> left_join(extra_cov_dat)
  }
...
prepare_pomp_model <- function(
  ...
  covariates <- pomp::covariate_table(
    dat_cov |> dplyr::select(time, pop, dpopdt, contains("cov"), contains("season")),
    times = "time"
  )
  ...
  po <- pomp(
    data = dat_mal |> dplyr::select(time, PF),
    params = param,
    times = "time",
...
run_fitting <- function(po, n_cores, parameters,
  ...
  if (is.null(mif_control)) mif_control <- list(Np = 1000, Nmif = 50, cooling.type = "geometric", cooling.fraction.50 = 0.5)
  if (is.null(pfilter_control)) pfilter_control <- list(Nrep = 10, Np = 1000)
...
prepare_model <- function(parameters,
  ...
  if (!is.null(actual_end_year) && !is.null(actual_end_month)) {
    dat <- preprocess_data_prediction(
      dat, model, covariate, start_year,
      end_year, start_month, end_month,
      end_year, end_month,
      window_start, window_end,
      actual_end_year, actual_end_month,
      extra_cov
    )
    dat_cov <- dat_mal_raw <- dat <- rbind(dat[[1]], dat[[2]])
  } else {
    dat <- preprocess_data(
      dat, model, covariate, start_year,
      end_year, start_month, end_month,
      window_start, window_end, extra_cov
    )
    dat_cov <- dat_mal_raw <- dat
  }
...
fit_model <- function(
  parameters, city,
  model = "const", covariate = "hadISD_VC",
  start_year = 1997, start_month = 1,
  end_year = 2014, end_month = 12,
  window_start, window_end,
  dataset, mode = "fit", allow_parallel = TRUE,
  ...
  manifest_args <- list(
    city = city, model = model, covariate = covariate,
...
#' Simulate Model
#'
#' This function simulates a model using the provided parameters and dataset.
#'
#' @param parameters A numeric vector or data frame containing the model parameters.
#' @param city The name of the city.
#' @param n_sims The number of simulations to perform (default is 1).
#' @param model The model to use (default is "original").
#' @param covariate The covariate to use (default is "RH2").
#' @param start_year The starting year of the simulation (default is 1997).
#' @param start_month The starting month of the simulation (default is 1).
#' @param end_year The ending year of the simulation (default is 2014).
#' @param end_month The ending month of the simulation (default is 12).
#' @param window_start The starting point of the window for preprocessing the data.
#' @param window_end The ending point of the window for preprocessing the data.
#' @param dataset The path to the dataset file.
#' @param output_format The format of the output (default is "data.frame").
#'
#' @return The simulated model output in the specified format.
#'
#' @details This function loads the dataset, preprocesses the data, and prepares the POMP model
#' using the provided parameters. It then performs the simulation and returns the result.
#'
simulate_model <- function(
  ...
  end_year = 2014, end_month = 12,
  window_start, window_end,
  dataset, output_format = c("data.frame", "pomps"), extra_cov = NULL,
  model_dir = "dynamical_model/fitting",
  model_file = NULL,
  validate_only = FALSE
) {
  ...
  po <- prepare_pomp_model(model, param, dat_cov, dat_mal, model_dir = model_dir, model_file = model_file, validate_only = validate_only)
  if (validate_only) {
    return(po)
  }
...
get_top_sample <- function(dataset, k = 1) {
  dataset2 <- dataset |>
    arrange(desc(loglik)) |>
    dplyr::select(-one_of("sample", "loglik", "loglik.se", "flag"))
  res <- dataset2[k, ] |>
    unlist() |>
    as.numeric()
...
plot_simulation <- function(dataset, mode = c("ribbon", "traj"), CI = 0.80, font_size = 12) {
  ...
  if (mode == "ribbon") {
    p <- dataset |> ggplot(aes(
      ...
      coord_cartesian(ylim = c(0, 20 * max_res))
  } else if (mode == "traj") {
    p <- dataset |> ggplot(aes(
...
preprocess_data_prediction <- function(
  ...
  window_start, window_end,
  actual_end_year,
  actual_end_month, extra_cov = NULL
) {
  if (is.null(actual_end_year)) {
    actual_end_year <- end_year
  }
  if (is.null(actual_end_month)) {
    actual_end_month <- end_month
  }
  n_years <- max(dat$year) - (min(dat$year) - 1)
  ...
  dat_extra_cov <- dat
  dat <- dat |>
    filter(year >= start_year, year <= actual_end_year) |>
    filter(year != start_year |
      (year == start_year & month >= start_month)) |>
    filter(year != actual_end_year | (year == actual_end_year & month <= actual_end_month))

  min_cov <- min(dat$covariate)
  max_cov <- max(dat$covariate)
  mean_cov <- mean(dat$covariate)
  sd_cov <- sd(dat$covariate)
  ...
  dat2 <- dat2 |>
    ...
    mutate(cov2 = sum(!!sym(covariate) * is_window) / n_month_window) |>
    ungroup() |>
    mutate(covariate = (cov2 - min_cov) / (max_cov - min_cov))

  if (end_month == 12) {
    start_year_prediction <- end_year + 1
    start_month_prediction <- 1
  } else {
    start_year_prediction <- end_year
    start_month_prediction <- end_month + 1
  }
  ...
  if (!is.null(extra_cov)) {
    ...
    for (i in 1:nrow(extra_cov)) {
      ...
      ex_cov <- df_extra |> pull(glue_name)
      max_ex_cov <- max(ex_cov)
      min_ex_cov <- min(ex_cov)

      df_extra <- dat_extra_cov |>
        filter(year >= start_year, year <= end_year_prediction) |>
        filter(year != start_year |
          (year == start_year & month >= start_month)) |>
        filter(year != end_year_prediction | (year == end_year_prediction & month <= end_month_prediction)) |>
        dplyr::select(c("year", "month", cov_name)) |>
        group_by(year) |>
        filter(month >= ws, month <= we) |>
        summarize("{glue_name}" := mean(!!sym(cov_name))) |>
        mutate("{glue_name}" := (!!sym(glue_name) - min_ex_cov) / (max_ex_cov - min_ex_cov))
      if (i == 1) {
        extra_cov_dat <- df_extra
      } else {
        extra_cov_dat <- extra_cov_dat |> left_join(df_extra)
    ...
    dat2 <- dat2 |> left_join(extra_cov_dat)
  }
  ...
  dat_training <- dat2 |>
    ...
    filter(year != end_year | (year == end_year & month <= end_month))
  dat_prediction <- dat2 |>
    filter(year >= start_year_prediction, year <= end_year_prediction) |>
    filter(year != start_year_prediction |
      (year == start_year_prediction & month >= start_month_prediction)) |>
    filter(year != end_year_prediction | (year == end_year_prediction & month <= end_month_prediction))
...
predict_model_states <- function(
  ...
  output_format <- match.arg(output_format)
  ## load dataset
  dat <- read_csv(dataset, show_col_types = FALSE)
  ...
  dat <- preprocess_data_prediction(
    ...
    start_month, end_month,
    end_year_prediction,
    end_month_prediction,
    window_start, window_end,
    actual_end_year,
    actual_end_month, extra_cov
  )

  dat_training <- dat[[1]]
  dat_prediction <- dat[[2]]
  ...
  dat_training2 <- rbind(dat_training, dat_prediction)

  po <- prepare_pomp_model(model, param, dat_training2, dat_training2)
  time(po) <- dat_training |> pull("time")
  training_result <- simulate(po,
    nsim = n_sims,
    params = param, format = "data.frame",
    include.data = TRUE
  )
  ...
  saved_hidden_states <- pfout |>
    ...
    group_by(.id) |>
    summarize(
      S1_0 = S1 / (S1 + E + I1 + I2 + S2),
      E_0 = E / (S1 + E + I1 + I2 + S2),
      I1_0 = I1 / (S1 + E + I1 + I2 + S2),
      I2_0 = I2 / (S1 + E + I1 + I2 + S2),
      S2_0 = S2 / (S1 + E + I1 + I2 + S2),
      K_0 = K,
      F_0 = F
    ) |>
    pivot_longer(-.id) |>
  ...
  dat_new <- rbind(dat_training, dat_prediction)


  pp <- parmat(unlist(param), ncol(saved_hidden_states))

  pp[rownames(saved_hidden_states), ] <- saved_hidden_states


  po2 <- prepare_pomp_model(model, param, dat_prediction, dat_prediction)
  # po2 <- po
  # time(po2) <- dat_prediction |> pull("time")
  timezero(po2) <- max(time(po))

  prediction_result <- po2 |> simulate(params = pp, format = "data.frame", include.data = TRUE)
  ...
  if (mode == "ribbon") {
    if (!by_pop) {
      p <- dataset |> ggplot(aes(
        ...
        ggdist::stat_lineribbon(.width = CI, linewidth = 0.8) +
        theme_bw(font_size) +
      ...
      p <- dataset |> ggplot(aes(
        x = time, y = PF / (pop / pop_division), fill = type, color = type
      )) +
        ggdist::stat_lineribbon(.width = CI, linewidth = 0.8) +
        theme_bw(font_size) +
        geom_vline(xintercept = max(training$time), linetype = "dashed") +
        theme(legend.position = "bottom", axis.title.y = element_markdown()) +
        labs(
          x = "Time", y = paste0("Observed *P. falciparum* cases\n", "per ", pop_division, " people"), color = "",
          fill = ""
        ) +
        theme(legend.position = "bottom") +
        scale_color_manual(
        ...
        coord_cartesian(ylim = c(0, 1.7 * max_res / (max_pop / pop_division)))
    }
  } else if (mode == "traj") {
    p <- dataset |> ggplot(aes(
      x = time, y = PF, color = type, group = .id
...
pfilter_initial_states <- function(pomp_model, param, n_sims) {
  ...
  initial_names <- c("S1_0", "E_0", "I1_0", "I2_0", "S2_0", "K_0", "F_0")
  init_conds <- c("S1_0", "E_0", "I1_0", "I2_0", "S2_0")
  state_names <- c("S1", "E", "I1", "I2", "S2", "K", "F")

  saved_states <- pfout@saved.states[[length(pfout@saved.states)]][state_names, ]
  saved_states <- rowMeans(saved_states)
  names(saved_states) <- initial_names

  saved_states[init_conds] <- saved_states[init_conds] / sum(saved_states[init_conds])

  param2[initial_names] <- saved_states
  return(param2)
}
...
filter_by_year_month <- function(dataset, y_s, y_e, m_s = NULL, m_e = NULL) {
  ...
  dat <- dataset |>
    filter(year >= y_s, year <= y_e) |>
    filter(year != y_s | (year == y_s & month >= m_s)) |>
    filter(year != y_e | (year == y_e & month <= m_e))
  return(dat)
}
...
separate_pfilter_simulation_datasets <- function(
  ...
  training_df <- filter_by_year_month(dataset, start_year, end_year, start_month, end_month)

  end_date <- make_date(end_year, end_month, 1)
  end_prediction_date <- make_date(end_year_prediction, end_month_prediction, 1)
  simulation_dates <- seq(end_date %m+% months(1), end_prediction_date, by = "year")
  pfilter_dates_start <- seq(end_date %m+% months(1) - years(prediction_window),
    end_prediction_date - years(prediction_window),
    by = "year"
  )
  pfilter_dates_end <- pfilter_dates_start + years(prediction_window) - months(1)
  #### change first pfilter date to be the beginning of the dataset
  pfilter_dates_start <- c(make_date(start_year, start_month, 1), pfilter_dates_start)
  pfilter_dates_end <- c(pfilter_dates_start[2] - months(1), pfilter_dates_end)
  partial_pfilter_dates_end <- pfilter_dates_end - years(prediction_window - 1)


  simulation_df <- simulation_dates |> lapply(function(x) {
    filter_by_year_month(dataset, x, x + months(11))
  })
  pfilter_df <- seq_along(pfilter_dates_start) |>
    lapply(function(i) {
      filter_by_year_month(dataset, pfilter_dates_start[i], pfilter_dates_end[i])
    })
  partial_pfilter_df <- seq_along(pfilter_dates_start) |>
    lapply(function(i) {
      filter_by_year_month(dataset, pfilter_dates_start[i], partial_pfilter_dates_end[i])
    })
  return(list(
    training_df = training_df,
    pfilter_df = pfilter_df,
    partial_pfilter_df = partial_pfilter_df,
...
prediction_by_window <- function(
  ...
  window_start, window_end,
  parameters_to_fit,
  dataset, output_format = c("data.frame", "pomps"), ignore_covid = TRUE,
  ...
  dat_training <- dat[[1]]
  dat_prediction <- dat[[2]]
  dat_complete <- rbind(dat_training, dat_prediction)
  complete_data <- dat_complete
  if (ignore_covid) {
    dat_complete <- dat_complete |> mutate(PF = if_else(year >= 2020 & year < 2021, NA, PF))
  }
  ...
  param2 <- param
  profile_vars <- names(param)
  profile_vars <- profile_vars[!profile_vars %in% parameters_to_fit]

  rdd <- generate_rw_sd(0.02, 0.1, model = model, profile_var = profile_vars)

  # simulates training part
  po <- prepare_pomp_model(model, param, dat_training, dat_training)
  training_result <- simulate(po,
    nsim = n_sims,
    params = param, format = "data.frame",
    include.data = TRUE
  ...
  pf_simulation_dfs <- separate_pfilter_simulation_datasets(
    dat_complete,
    start_year, start_month,
    end_year, end_month,
    end_year_prediction, end_month_prediction, prediction_window
  )

  ## run first year separately
  ## puts dataset "pointer" at initial point of simulation
  i <- 1
  dataset <- pf_simulation_dfs$pfilter_df[[i]]
  po <- prepare_pomp_model(model, param, dataset, dataset)
  param2 <- pfilter_initial_states(po, param, n_sims)

  # #   ## puts dataset "pointer" a year ahead
  # dataset_partial <- pf_simulation_dfs$partial_pfilter_df[i]
  # po <- prepare_pomp_model(model, param, dataset)
  # param2 <- pfilter_initial_states(po, param, n_sims)


  res <- foreach(i = 2:length(pf_simulation_dfs$pfilter_df)) %do% {
    ...
    # ## simulate next year
    dataset_simulation <- pf_simulation_dfs$simulation_df[[i - 1]]

    po2 <- prepare_pomp_model(model, param2, dataset_simulation, dataset_simulation)
    prediction_result <- simulate(po2,
      nsim = n_sims,
      params = param2, format = "data.frame",
      include.data = TRUE
    )
    ...
    # ## fit using dataset 1 year ahead, only if not last year

    list(prediction = prediction_result, param_fit = param)
  }

  prediction <- list(training = training_result)

  training <- list(training = training_result)
  prediction <- lapply(res, function(x) {
    return(x$prediction)
  })

  parameters_prediction <- list(training = parameters)
  parameters_prediction <- append(parameters_prediction, lapply(res, function(x) {
    return(x$param_fit)
  }))

  result <- list(
    training = training, prediction = prediction,
    parameters_prediction = parameters_prediction, ignore_covid = ignore_covid,
...
simulate_yearly_prediction <- function(
  ...
  # po <- prepare_pomp_model(model, param, dataset)
  # param2 <- pfilter_initial_states(po, param, n_sims)
  # print(pf_simulation_dfs$simulation_df)

  res <- foreach(i = 2:nrow(parameters)) %do% {
    # print(i)
    # if (i < length(pf_simulation_dfs$pfilter_df)) {
    ...
    # ## simulate next year
    dataset_simulation <- pf_simulation_dfs$simulation_df[[i - 1]]
    po2 <- prepare_pomp_model(model, param, dataset_simulation, dataset_simulation)
    prediction_result <- simulate(po2,
      nsim = n_sims,
      params = param, format = "data.frame",
      include.data = TRUE
...
plot_prediction_by_year <- function(result, mode = c("ribbon", "traj"), CI = 0.8, font_size = 12) {
  ...
  if (mode == "ribbon") {
    p <- dataset |> ggplot(aes(
      ...
      coord_cartesian(ylim = c(0, 1.7 * max_res))
  } else if (mode == "traj") {
    p <- dataset |> ggplot(aes(
      x = time, y = PF, color = type, group = .id
    )) +
